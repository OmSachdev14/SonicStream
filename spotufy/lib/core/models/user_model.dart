...
class UserModel {
  ...
  final String email;
  final String token;
  final List<FavSongsModel> favorites;
  UserModel({
    required this.id,
    required this.name,
    required this.email,
  ...
  factory UserModel.fromMap(Map<String, dynamic> map) {
    return UserModel(
      ...
      email: map['email'] ?? "",
      token: map['token'] ?? "",
      favorites: List<FavSongsModel>.from((map['favorites']?? []).map((x) => FavSongsModel.fromMap(x as Map<String,dynamic>),),),
    );
  }
